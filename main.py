...
camera.fov = 9
world = Entity(model='quad', texture='street',scale=60, z=1, tag='world')


player = Entity(position=(0,-8))
man = Animation("walking", parent = player, autoplay=False)
anim = Animator( animations = {
  'idle': Entity(parent=player, model='cube',texture='walking_0',),
  'walking': Animation("walking", parent=player, autoplay=False)
})
...
for i in [-8,8]:
  for j in [-2, 4]:
    house  = Sprite(model='quad',texture='house',scale=0., collider = 'box',
                    position=(i,j,0), rotation_z=0 if i ==-8 else 180, tag="house")
  for i in [-2.5, 2.5]:
    for j in [-3.5, 6]:
      house = Sprite(model='quad', texture='house', scale=0.75, collider='box',
                     position=(i, j, 0), rotation_z=270 if j ==-3.5 else 90,tag="house")

follow = SmoothFollow(target=player ,offset=[0,0,-4] )
...
for i in range(12):
  if i < 6:
    ...
    rot = 0
    val = 1
  npc = Animation("npc", x=4, autoplay=True, rotation_z=rot,
                  collider='box', scale=0.75, position=(r.randint(-22,22),r.randint(-22,22)),tag='npc')
  npcs.append((npc,val))


car = Entity(model='quad',texture='car',collider='box', scale=(2,1),
  ...
  if car_mode:
    ...
    if not head_ray.hit or back_stuck or (head_ray.hit and head_ray.entity.tag == 'npc'):
      ...
      if head_ray.hit and head_ray.entity.tag == 'npc':
        Entity(model="quad", texture='corpse', color=color.random_color(),
